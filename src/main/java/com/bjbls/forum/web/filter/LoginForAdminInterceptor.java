package com.bjbls.forum.web.filter;
   ...
   /*
    ...
    public boolean preHandle(HttpServletRequest request,
        ...
        if(admin!=null){
            ...
            admin=adminService.login1(admin.getUsername(), admin.getPassword());
            if(admin!=null){
                flag=true;
        ...
        if(flag==false){//如果没有登录或登录异常
            if (request.getHeader("x-requested-with") != null && request.getHeader("x-requested-with").equalsIgnoreCase("XMLHttpRequest")){//如果是ajax请求
                ...
                response.sendRedirect(basePath+"/system/index?myMessage="+message);
				/*redirect:
